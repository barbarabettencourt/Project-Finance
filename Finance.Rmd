...
ggplot(airlines, aes(x = date, y = Returns)) + 
  ...
  ggtitle("Plot of Returns of United Airlines and American Airlines") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("")

rm(airlines)

#carslberg, harbo, asahi
...
ggplot(beers, aes(x = date, y = Returns)) + 
  geom_line(aes(color = Stocks)) + 
  scale_color_manual(values = c("skyblue4", "orchid1")) +
  ggtitle("Plot of Returns of Carlsberg and Asahi") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("")
...
#From the histograms the returns appear to have the normal distribution shape, where most of them appear unimodal. There appears to be a few that are skewewd, which can also be observed in the boxplots.
...
#normal <- 1,2,3,4,9,10,12,14,8 barelly with 0.05991
#not normal <- 5,6,7,11,13,15

```

9.You should also provide an equity curve for each asset (that is, a
...
10. Fit different distributions to your data, which one fits better?

```{r}

```

11.  Compute Sharpe’s slope for each asset. Which asset has the highest slope?

```{r}

```

12. Convert the monthly sample means
...
13. Construct pairwise scatter plots between your assets returns and comment on
...
14. You should also compute the sample covariance matrix of the
