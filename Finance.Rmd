...
Skewness <- rep(NA,15)
Kurtosis <- rep(NA,15)
table <- data.frame(Company,Means,Sd,Skewness,Kurtosis)
...
#airlines
plot(equityCurve[,1], equityCurve[,2], type = "l", ylim= c(0.2,6), xlab="", ylab="Equity Curves", main="Airlines")
for(i in 1:6)
{
...
#beers
plot(equityCurve[,1], equityCurve[,9], type = "l", ylim= c(0.4,3), xlab="", ylab="Equity Curves", main="Beers")
for(i in 1:7)
{
...
{
  names <- list(stock_names)
  hist(returns[,i+1], xlab="", main = names[[1]][i])
}
...
#maybe try in ggplot make it look better

#airlines
boxplot(returns[,2:8])

#beers
boxplot(returns[,8:16])
